...
public class HandlerDate {
     ...
     *
     */
    private final String datesBuilder[][] = {
    ...
    public String displayDate() {
        ...
        int iDay;
        iDay = 0;
        int dayNumber; // iMonth, iDay => index
        dayNumber = 0;
        String resultat = "Ce n'est pas une bonne entrée";
        String resultStr = "Le %dème jour correspond au %s %s %s.\n";

        // System.out.print("Choisir un numéro de jour de l'année [1-365] : ");
        // this display is moved into jsp file
        if (nbOfDay >= 1 && nbOfDay <= 366) {
            ...
            // TODO : remplacement du System.out.printf par un String.Format(..,..,..,)
            ...
            // );
            resultat = String.format(resultStr, nbOfDay, datesBuilder[ARRAY_OF_DAYS][iDay],
                    (dayNumber == 1 ? dayNumber + "er" : dayNumber), datesBuilder[ARRAY_OF_MONTHS][iMonth]);
            // System.out.println(" (Sous-entendu : lundi est le premier jour de
            // l'année.)\n\n");
    ...
    private int searchMonth() {
        ...
        stop:
        for (int i = 0; i < is30DaysMonth.length; i++) {
            switch (i) {
                case 0:
